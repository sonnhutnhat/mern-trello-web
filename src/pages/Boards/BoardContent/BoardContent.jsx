...
function BoardContent({ board, createNewColumn, createNewCard, moveColumns, moveCardInTheSameColumn, moveCardToDifferentColumn }) {
  ...
  useEffect(() => {
    // Columns đã được sắp xếp ở component cha cao nhất (board/_id.jsx) (Vid 71)
  ...
  const moveCardBetweenDifferentColumns = (
    ...
    setOrderedColumnsState(prevColumns => {
      ...
      if (nextOverColumn) {
        ...
        // Xóa cái Placeholder Card đi nế nó đang tồn tại
      ...
      // Phải dùng tới activeDragItemData.columnId hoặc tốt nhất là oldColumnWhenDragginngCard._id (set vào state từ bước handleDragStart) chứ không phải activeData trong scope handleDragEnd này vì sau khi đi qua onDragOver và tới đây là state của card đã bị cập nhật một lần rồi.
      if (triggerFrom === 'handleDragEnd') {
        moveCardToDifferentColumn(activeDraggingCardId,
          oldColumnWhenDragginngCard._id,
          nextOverColumn._id,
          nextColumns
        )
      }
  ...
  const handleDragEnd = (event) => {
    ...
    if (activeDragItemType === ACTIVE_DRAG_ITEM_TYPE.CARD) {
      ...
      if (oldColumnWhenDragginngCard._id !== overColumn._id) {
        ...
        const dndOrderedCards = arrayMove(oldColumnWhenDragginngCard?.cards, oldCardIndex, newCardIndex)
        ...
        moveCardInTheSameColumn(dndOrderedCards, dndOrderedCardIds, oldColumnWhenDragginngCard._id)
      }
    }
  ...
  const collisionDetectionStrategy = useCallback((args) => {
    ...
    if (overId) {
      // Nếu cái over nó là column thì sẽ tím tới cái cardId gần nhất bên trong khu vực va chạm đó dựa vào thuật toán phát hiện va chạm closestCenter hoặc closestCorners đều được. Tuy nhiên ở đây dùng closestCorners mượt mà hơn
